...
1. **Simplex:** Ocorre em uma direção fixa, com transmissor e receptor (ex.: transmissão de rádio ou TV).
2. **Half-Duplex:** Bidirecional, mas não simultaneamente. Os dispositivos se refezam entre transmissor e receptor (ex.: walkie-talkie)
3. **Full-Duplex:** Bidirecional simultâneo. Nesse caso requer dois canais físicos separados (um par de fios trançados ou dois cabos ópticos) ou multiplexação de frequência no caso de fibra ópica. (ex.: switch moderno, telefonia, ethernet via par trançado cat5e+).
## Referências
