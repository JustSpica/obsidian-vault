...
Em um switch nunca antes configurado, ele possui apenas a VLAN 1 com todas as portas, ou seja, uma bridge:

- Sem criar outras VLANs, eu precisaria usar mais de um switch para suportar duas redes sem quebrar o domínio de broadcast
- VLANs me permitem criar domínios de broadcast em um mesmo switch (ex.: rede A = VLAN 2, rede B = VLAN 3...).
...
VLANs são identificadas por números e não existe ligação entre uma VLAN e outro nível de enlance, logo um domínio de broadcast não consegue se comunicar com outro na camada de enlace.
...
O 802.1q altera o quadro Ethernet adicionando **4 bytes** entre o MAC de origem e o antigo campo EtherType/Length.

**Exemplo do cabeçalho 802.1q:**![[998 Imagens/exemplo_cabecalho_802.1q.png]]
...
O antigo campo Type/Length do quadro Ethernet continua existindo, mas após a inserção da tag, ele passa a representar o tipo original do quadro (ex.: IPv4 `0x0800`, ARP `0x0806`, etc.).
...
Por isso é comum o switch inserir a tag ao encaminhar para um trunk e remover a tag ao entregar em access.
