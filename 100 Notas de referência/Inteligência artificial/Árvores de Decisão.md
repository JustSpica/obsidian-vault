...
Árvores de decisão são modelos preditivos que representam uma sequência de **testes encadeados sobre atributos** até chegar a uma decisão final. Elas servem para classificação e regressão, produzindo regras interpretáveis no formato `IF-ELSE`.
...
A ideia é buscar regiões com menor impureza possível. Em classificação, uma região pura contém instâncias de uma única classe. Em regressão, uma região pura contém valores alvo próximos.
...
A indução é o processo de construir a árvore a partir dos dados. O procedimento mais comum é top-down: começa na raiz e divide o conjunto recursivamente.

O algoritmo de Hunt segue esta lógica:

1. Se todas as instâncias que chegam ao nó pertencem à mesma classe, o nó vira folha.
...
- **Homogeneidade das instâncias:** Os atributos das instâncias são iguais.
...
- **Overfitting:** Podem memorizar detalhes do treino.
